/**
 * KisanMarg Unified API & Fallback Client
 * Seamlessly talks to FastAPI backend if available, or computes calculations
 * client-side using bundled Gujarat APMC dataset for offline & instant Vercel static hosting.
 */
...
const ApiClient = (function () {
  ...
  async function loadFallbackData() {
    ...
    try {
      ...
      console.warn("Could not fetch ./data/mandis_gujarat.json directly, using inline data if available", err);
      if (window.FALLBACK_DATA_INLINE) {
        cachedFallbackData = window.FALLBACK_DATA_INLINE;
        return cachedFallbackData;
      }
      throw err;
    }
    ...
    async calculateNetProfit(params) {
      ...
      if (isOnline) {
        try {
          ...
          console.warn("Backend call failed, using client engine", e);
        }
      }
      const data = await loadFallbackData();
    ...
    async getQueues() {
      ...
      return {
        queues: data.mandis.map(m => ({
          ...
          gate_advisory: "Peak arrivals 9:00 AM - 11:30 AM. Token system active."
        }))
      };
    },
